...
#
# Phase 2.7 v5 fix — bpp + gamma:
#   - bpp=32 (RGBA, alpha-capable) NOT bpp=24 (RGB, no alpha). TORCS's
#     menu buttons use alpha-blended dark gradients with outlined text.
#     Under Mesa software rendering on Xvfb with no alpha channel, the
#     blend silently falls back to opaque white — producing the "white
#     blocks over every menu label" symptom reported on 2026-05-13.
#   - gamma=1.0 (neutral) NOT gamma=2.0. A 2× gamma correction baked
#     into the GL context darkens and over-saturates everything, which
#     is what produced the deep-blue tint over the scene.
# Stock TORCS ships these values; deviating from them broke the GUI.
#
...
echo "[init] writing /root/.torcs/config/screen.xml (fullscreen=yes, bpp=32, gamma=1.0)"
mkdir -p /root/.torcs/config
cat > /root/.torcs/config/screen.xml <<'EOF'
<?xml version="1.0" encoding="UTF-8"?>
<!-- Generated by OVERRIDE torcs_container_init.sh — Phase 2.7 v5.
     Schema + values match /usr/local/torcs/share/games/torcs/config/screen.xml.
     Only `fullscreen` and the dimensions are deliberately changed from
     stock; bpp + gamma are kept neutral to preserve menu/HUD rendering. -->
