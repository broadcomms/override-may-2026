...
#
# Two-part patch:
#   1. `1920x1080x24+32` — keep 24-bit color depth (x11vnc compatibility)
#      but pad to 32 bits per pixel so an 8-bit alpha plane is allocated.
...
sed -i 's|Xvfb :1 -screen 0 1920x1080x24 &|Xvfb :1 -screen 0 1920x1080x24+32 -ac +extension RENDER +extension GLX \&|' \
...
echo "[init] writing /root/.torcs/config/screen.xml (fullscreen=yes, 1920x1080, bpp=32, gamma=1.5)"
...
<!-- Generated by OVERRIDE torcs_container_init.sh — Phase 2.7 v6.
     ...
     LLVMpipe's reliable RGBA-texture-size envelope; otherwise TORCS's
     PUI menu widgets render as opaque white blocks. See the comment
     block above this `cat` heredoc for the full diagnosis. -->
