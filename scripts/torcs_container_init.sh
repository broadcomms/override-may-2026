...
XFDESKTOP_EOF
    ...
    # ── (e) Auto-launch TORCS on XFCE session start ──────────────────────────
    # ~/.config/autostart/ files are executed by xfce4-session at login.
    # No "Mark Trusted" prompt — that dialog applies only to xfdesktop file
    # icons, not to session-autostart entries.
    # DISPLAY=:1 is explicit so the torcs binary always targets the Xvfb
    # display regardless of which shell inherits this env. The 2-second sleep
    # lets XFCE finish compositing before TORCS opens its GL context.
    cat > /root/.config/autostart/torcs-kiosk.desktop <<'AUTOSTART_EOF'
[Desktop Entry]
Version=1.0
Type=Application
Name=TORCS Kiosk Auto-Launch
Comment=Auto-start TORCS in fullscreen for OVERRIDE kiosk mode
Exec=bash -c "sleep 2 && DISPLAY=:1 /usr/local/torcs/bin/torcs"
Terminal=false
Hidden=false
X-XFCE-Autostart-Enabled=true
AUTOSTART_EOF

    echo "[init] kiosk surface configured — panels suppressed, icons hidden, TORCS autostart queued"
    if [ -f "$BRAND_IMAGE" ]; then
