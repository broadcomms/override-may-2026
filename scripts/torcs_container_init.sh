...
#
# Two-part patch:
#   1. `1280x800x24+32` — drop resolution to 1280x800 (16:10, within LLVMpipe's
...
PANEL_EOF
    ...
    BRAND_IMAGE_STAGED=/usr/local/share/backgrounds/override-kiosk.svg
    BRAND_IMAGE=""
    if [ -f "$BRAND_IMAGE_MOUNT" ]; then
        mkdir -p /usr/local/share/backgrounds
        cat > "$BRAND_IMAGE_STAGED" <<'SVG_EOF'
<svg xmlns="http://www.w3.org/2000/svg" width="1280" height="800" viewBox="0 0 1280 800">
  <rect width="1280" height="800" fill="#111827"/>
  <rect x="0" y="0" width="1280" height="800" fill="url(#glow)"/>
  <g opacity="0.22">
    <path d="M-120 210 L360 40" stroke="#ff5b21" stroke-width="18" stroke-linecap="round"/>
    <path d="M-40 300 L470 120" stroke="#ff7a3d" stroke-width="8" stroke-linecap="round"/>
    <path d="M860 760 L1320 600" stroke="#ff5b21" stroke-width="14" stroke-linecap="round"/>
  </g>
  <g opacity="0.95">
    <text x="640" y="408" text-anchor="middle"
          font-family="DejaVu Sans, Arial, sans-serif"
          font-size="170" font-weight="800" letter-spacing="8"
          fill="#f4f4f5">OVERRIDE</text>
    <text x="640" y="458" text-anchor="middle"
          font-family="DejaVu Sans Mono, monospace"
          font-size="22" letter-spacing="9"
          fill="#9ca3af">COCKPIT STANDBY</text>
  </g>
  <defs>
...
SVG_EOF
    ...
    cat > /root/.config/xfce4/xfconf/xfce-perchannel-xml/xfce4-desktop.xml <<XFDESKTOP_EOF
<?xml version="1.0" encoding="UTF-8"?>
<!-- Generated by OVERRIDE torcs_container_init.sh — kiosk mode.
...
${BACKDROP_IMAGE_LINE}
  ...
  <property name="last" type="empty">
    ...
    <property name="window-height" type="int" value="800"/>
  </property>
</channel>
XFDESKTOP_EOF
...
PAUSE_FILE=/tmp/override-managed-race.lock
...
LOOP_EOF
    ...
    # ── (f) Apply kiosk settings after XFCE session start ────────────────────
    # Pre-seeding XML alone is not enough in this lab image: XFCE writes its
    # own defaults on first session boot, which was reintroducing the stock
    # wallpaper, File System/Home icons, and a panel work-area reservation.
    #
    # This script runs inside the live XFCE session (so xfconfd and DBus are
    # available) and force-applies the kiosk settings to the correct
    # `xfce4-desktop` channel. It also removes the manual launcher icon in
    # kiosk mode because TORCS now auto-starts via the supervisor.
    cat > /usr/local/bin/override-kiosk-apply.sh <<KIOSK_APPLY_EOF
#!/usr/bin/env bash
set -e
...
set_bool() {
    local path="\$1"
    local value="\$2"
    xfconf-query -c xfce4-desktop -p "\$path" -s "\$value" 2>/dev/null || \
        xfconf-query -c xfce4-desktop -p "\$path" -n -t bool -s "\$value"
}
...
set_int() {
    ...
    local value="\$2"
    xfconf-query -c xfce4-desktop -p "\$path" -s "\$value" 2>/dev/null || \
        xfconf-query -c xfce4-desktop -p "\$path" -n -t int -s "\$value"
}
...
set_string() {
    ...
    local value="\$2"
    xfconf-query -c xfce4-desktop -p "\$path" -s "\$value" 2>/dev/null || \
        xfconf-query -c xfce4-desktop -p "\$path" -n -t string -s "\$value"
}

monitor_name=\$(xrandr --current 2>/dev/null | awk '/ connected/{print \$1; exit}')
if [ -z "\$monitor_name" ]; then
    monitor_name=screen
fi
monitor_key="monitor\${monitor_name}"

for workspace in 0 1 2 3; do
    base="/backdrop/screen0/\${monitor_key}/workspace\${workspace}"
    set_int "\${base}/color-style" 0
    set_int "\${base}/image-style" ${BACKDROP_IMAGE_STYLE}
    set_string "\${base}/last-image" "${BRAND_IMAGE_STAGED}"
done

set_bool /desktop-icons/file-icons/show-filesystem false
set_bool /desktop-icons/file-icons/show-home false
set_bool /desktop-icons/file-icons/show-trash false
set_bool /desktop-icons/file-icons/show-removable false
set_bool /desktop-menu/show false
set_bool /windowlist-menu/show false

rm -f /root/Desktop/launch-torcs.desktop
...
KIOSK_APPLY_EOF
    chmod +x /usr/local/bin/override-kiosk-apply.sh

    # ── (g) Autostart entries ────────────────────────────────────────────────
    cat > /root/.config/autostart/override-kiosk-apply.desktop <<'KIOSK_APPLY_DESKTOP_EOF'
[Desktop Entry]
Version=1.0
Type=Application
Name=OVERRIDE Kiosk Apply
Comment=Apply OVERRIDE kiosk wallpaper, icon suppression, and panel removal
Exec=/usr/local/bin/override-kiosk-apply.sh
Terminal=false
Hidden=false
X-XFCE-Autostart-Enabled=true
OnlyShowIn=XFCE;
KIOSK_APPLY_DESKTOP_EOF

    # ── (h) Autostart pointing to the supervisor ─────────────────────────────
...
AUTOSTART_EOF
    ...
    # ── (i) Strip window decorations via xfwm4 ───────────────────────────────
...
#
# Phase 2.7 v7 — resolution dropped from 1920x1080 to 1280x800.
#
...
#
# 1280x800 stays well within LLVMpipe's reliable range while restoring a
# little more vertical room for TORCS's menu stack and race HUD. The UI
# frame uses a matching 16:10 wrapper so the simulator gets more height
# without stretching.
#
...
echo "[init] writing /root/.torcs/config/screen.xml (fullscreen=yes, 1280x800, bpp=32, gamma=1.5, OVERRIDE menu theme)"
mkdir -p /root/.torcs/config
cat > /root/.torcs/config/screen.xml <<'EOF'
<?xml version="1.0" encoding="UTF-8"?>
<!-- Generated by OVERRIDE torcs_container_init.sh — Phase 2.7 v7.
     Schema matches /usr/local/torcs/share/games/torcs/config/screen.xml.
     1280x800 stays within LLVMpipe's reliable RGBA-texture-size envelope;
