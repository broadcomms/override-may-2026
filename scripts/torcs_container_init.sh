...
# small window inside the XFCE desktop. Without this, the noVNC iframe
# was streaming a 1920x1080 desktop with a ~600x350 TORCS window inside,
# double-scaled down to whatever iframe size — HUD text effectively 8-10
# px and unreadable. Fullscreen eliminates the inner XFCE chrome from
# the stream: TORCS owns the whole framebuffer, the iframe shows pure
# 3D race chrome at full pixel density.
#
# Idempotent — only writes when the file is absent OR doesn't already
# carry fullscreen=yes; respects an operator's hand-edits.
echo "[init] writing /root/.torcs/config/screen.xml (fullscreen=yes for legible noVNC)"
mkdir -p /root/.torcs/config
SCREEN_XML=/root/.torcs/config/screen.xml
if [ ! -f "$SCREEN_XML" ] || ! grep -q 'val="yes"' "$SCREEN_XML" 2>/dev/null; then
    cat > "$SCREEN_XML" <<'EOF'
<?xml version="1.0" encoding="UTF-8"?>
<!-- Generated by OVERRIDE torcs_container_init.sh — see Phase 2.7 ADR-004 -->
<!DOCTYPE params SYSTEM "params.dtd">
<params name="Screen">
  <section name="Properties">
    <attnum name="window width" val="1920"/>
    <attnum name="window height" val="1080"/>
    <attnum name="bpp" val="32"/>
    <attstr name="fullscreen" val="yes"/>
    <attstr name="VInit" val="best"/>
    <attstr name="maximum color depth" val="yes"/>
    <attstr name="display" val="hardware"/>
  </section>
</params>
EOF
fi
