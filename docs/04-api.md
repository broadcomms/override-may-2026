...
### 4.16 `GET /api/torcs/control-status` (Phase 2)

Reports whether the in-container TORCS control daemon is configured and reachable. Always returns 200 — the UI uses `enabled` + `reachable` booleans to branch between "render Start/Stop buttons", "render disabled hint", and "hide entirely". See ADR-004 for the security model.

**Response 200** — `TorcsControlPlaneStatus`:
...
- `enabled` — true when both `TORCS_CONTROL_URL` and `TORCS_CONTROL_SECRET` are set in the override service's environment.
- `reachable` — true when the daemon's `/health` responded 200 within the 3 s probe timeout.
- `active` — proxied from the daemon's `/control/status.active`; true when a gym_torcs subprocess is currently running.
- `session_id` — set only when a race is active; matches the session_id the daemon was given on `/control/start`.
- `detail` — non-null only when `enabled=false` or `reachable=false`; a human-readable hint the UI surfaces verbatim.

### 4.17 `POST /api/torcs/start-race` (Phase 2)

Proxies to the in-container daemon's `/control/start` after OVERRIDE generates the canonical `session_id`. UI calls this when the operator clicks "Start race" on `/upload`.
...
All fields optional:
- `track` — `^[a-z0-9_-]+$`, ≤ 40 chars. Defaults to `"aalborg"`.
- `laps` — int [1, 200]. Defaults to 10.
- `track_name`, `notes` — operator-supplied hints echoed back in the response so the UI can pre-fill them on the eventual `POST /api/sessions/torcs-live` call.

**Response 201**

```json
{
  "session_id": "s_torcs_live_1715625321_a4f9",
  "pid": 9999,
  "telemetry_dir": "/home/student/workspace/gym_torcs/telemetry/",
  "track": "aalborg",
  "laps": 10,
  "track_name_hint": "Aalborg",
  "notes_hint": "calibration lap, baseline setup"
}
```

`session_id` is generated by the OVERRIDE proxy (not the caller) — the path-traversal surface is kept to one regex on the daemon side.

**Error codes:**
- `503 CONTROL_DISABLED` — override service has no `TORCS_CONTROL_URL/SECRET` configured. UI normally hides Start/Stop buttons in this state.
- `503 CONTROL_UNREACHABLE` — daemon connection/timeout. The torcs container may still be booting.
- `409 RACE_ACTIVE` — daemon already has an active race; caller must stop it first (idempotent: see §4.18).
- `502 CONTROL_FAILED` — unexpected non-2xx from the daemon (the response body is surfaced in `detail` truncated to 300 chars).
- `422` — Pydantic validation rejected the payload (e.g. malformed `track`).

### 4.18 `POST /api/torcs/stop-race` (Phase 2)

Proxies to `/control/stop`. **Idempotent**: returns 200 with `status="no_active_race"` when nothing is running — UI fires-and-forgets without first checking status.

**Request body**: none.

**Response 200** (active race stopped):
...
**Response 200** (no race was running):

```json
{ "status": "no_active_race", "session_id": null, "exit_code": null }
```

`exit_code = -15` is the typical SIGTERM return from gym_torcs; `-9` indicates the daemon's SIGKILL fallback fired after the 5 s grace expired. See ADR-004 §security for the two-stage termination guarantee.

### 4.19 `DELETE /api/sessions/{session_id}`
