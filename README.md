...
| **CI workflows** | Not in v1.0 scope. Quality gate today: `pytest -q -m "not network"` (323 green) + `npm run typecheck && npm run build` per the T-72h pre-flight in `docs/plans/final-lock-checklist.md` | `.github/workflows/` (empty placeholder removed in 3.1) |
